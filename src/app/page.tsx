...
// Mapping des couleurs par ID de catégorie (pour les surlignements)
...
// Mapping des couleurs d'icônes par ID (pour les icônes)
    ...
    <div className="flex flex-col min-h-screen font-sans bg-gray-50">
      ...
      <div className="relative py-8 px-4 z-40 bg-gray-50">
        <div className="max-w-6xl mx-auto">
         ...
         {/* Transaction Type Tabs with Deposit Button - CENTRÉ SUR LA MÊME LIGNE */}
<div className="flex justify-center items-center gap-6 mb-6">
    ...
    {['all','rent','buy'].map((type) => (
      <button
        ...
        className={cn(
          ...
          searchType === type ? "bg-[#003B4A] text-white shadow-md" : "text-gray-500 hover:bg-gray-50 hover:text-gray-900"
        )}
      >
        {type === 'all' ? 'TOUS' : type === 'rent' ? 'LOUER' : 'ACHETER'}
      </button>
    ))}
  </div>
  ...
  <Link href="/deposit">
    <Button className="bg-[#00BFA6] hover:bg-[#00908A] text-white rounded-full px-6 py-2.5 text-sm font-bold shadow-lg shadow-[#00BFA6]/20 transition-all flex items-center gap-2">
      <Plus className="h-4 w-4" /> Déposer votre annonce
    </Button>
  </Link>
</div>
              ...
              return (
                ...
                >
                  <div className="w-20 h-20 rounded-2xl flex items-center justify-center border-2 border-gray-100 bg-white shadow-md group-hover:shadow-lg transition-all">
                    <Icon className={cn("h-10 w-10", getIconColorById(category.id))} />
                  </div>
      ...
      {loading ? (
        ...
        Object.keys(groupedAnnounces).length === 0 ? (
          ...
          REAL_ESTATE_CATEGORIES.map((category) => {
            const catData = groupedAnnounces[category.id];
      ...
      {/* --- RESTE DU CODE IDENTIQUE (WHY CHOOSE US, CTA, ETC.) --- */}
